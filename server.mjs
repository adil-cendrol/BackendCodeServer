...
// const { OpusDecoder } = Prism;
// const decoder = new OpusDecoder({ frameSize: 960, channels: 1, rate: 48000 });
// ----------------------
...
// const decoder = new OpusDecoder({ rate: 48000, channels: 1 });
  ...
  ws.on("message", async (message) => {
    ...
    if (type === "offer") {
      ...
      await new Promise((resolve) => {
        if (pcClient.iceGatheringState === "complete") resolve();
        else pcClient.on("iceGatheringStateChange", () => {
          if (pcClient.iceGatheringState === "complete") resolve();
        });
      });

      // Send client answer including all ICE candidates
      ws.send(JSON.stringify(pcClient.localDescription));
      ...
      await new Promise((resolve) => {
        if (pcMeta.iceGatheringState === "complete") resolve();
        else pcMeta.on("iceGatheringStateChange", () => {
          if (pcMeta.iceGatheringState === "complete") resolve();
        });
      });
      console.log({
      ...
      console.log("✅ Sending SDP to Meta:", answerPayload);
      metaWs.send(JSON.stringify(answerPayload));
    }
  });
